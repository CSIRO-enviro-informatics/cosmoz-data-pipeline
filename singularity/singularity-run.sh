...
sudo singularity instance start --cleanenv --writable-tmpfs --bind "../mgdb:/data/db" --net --network bridge --hostname "cosmoz.mongodb" mongodb.sif cosmoz-mongo-inst
...
sudo singularity instance start --cleanenv --writable-tmpfs --bind "../tsdb:/var/lib/influxdb" --bind "../influxdb.conf:/etc/influxdb/influxdb.conf" --net --network bridge --hostname "cosmoz.influxdb" influxdb.sif cosmoz-influx-inst
...
echo "${T2_IP_ADDR}   influx.cosmoz.csiro.au     cosmoz.influxdb" >> ./dummy_hosts.txt
echo "${T1_IP_ADDR}   mongo.cosmoz.csiro.au      cosmoz.mongodb" >> ./dummy_hosts.txt
sudo singularity instance start --cleanenv --writable-tmpfs --net --network bridge --hostname "cosmoz.rest" --network-args "portmap=${COSMOZ_REST_API_HOST_PORT}:8080/tcp" --bind ./dummy_hosts.txt:/etc/hosts cosmoz-app.sif cosmoz-rest-inst
